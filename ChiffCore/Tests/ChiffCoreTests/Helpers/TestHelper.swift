...
class TestHelper {
    ...
    static let chiffRequestEncrypted = "nzLY8eBUZgQ4WYILTebsRVHGu12Tx3w85A6GXk6M-p0wNFZhPwkfTZctlOyQQ6Y-PKk_ghKpbKhW2p7M8syYr344UEQCjiaZxXocdz7r9PjnJ-ZE8kfye6-8XXZu1qbA4YedY02m92AWYftv_lFCj7v_9tX_4Co571F_muLaW6JVrKZrNz9XXlaf4WcrSu_id9zG3kGAlc-sztITZv-5_lHsrb58ffaOApl-v1kO5g8p4YUhbGKvDqxBq2ci0tAA4QqYPL42l4C4m6YLtH55GCEK5i3NwnsUZDeMH40x-H662I2gCjv6qNdvJ9MG4Sr9B8hDkrs9YUZA7swrpMWhkx4-TBlptOgNcbf8cUoGcDrJmRJ8ca69ZO7__zoEKIp2Q-Ev_sGICl9URggw7ZJgtC8iB2NTqj5448hmB7e04dB_DrOIjKl-q3ire7eqSj3Nn0IlynazMMsn2DcEtEOLKyPHuqwbeq6mRn2QHWUnRiHkCIsYdN3DFZ2p7ao"
    static let chiffRequest = ChiffRequest(accountID: TestHelper.userID, accountIDs: nil, browserTab: 0, challenge: "tnlOysO9SoL_PAXiEuqgSNZdnZ_BfJ_ri_9UnF4B5nM", password: "[jh6eAX)og7A#nJ1:YDSrD6#61cf${\"A", passwordSuccessfullyChanged: false, siteID: TestHelper.sampleSite.id, siteName: TestHelper.sampleSite.name, newSiteName: "new-\(TestHelper.sampleSite.name)", siteURL: TestHelper.sampleSite.url, notes: "Important note", type: .add, relyingPartyId: "test.com", algorithms: nil, username: TestHelper.username, sentTimestamp: Timestamp(TimeInterval()), count: 0, orderKey: nil, organisationName: nil, askToChange: nil, webAuthnExtensions: nil, sessionID: TestHelper.sessionID, accounts: nil) // Challenge is SHA256({"challenge":"3rEAfh7LQeqrLBiQjZM5284v54xIWEWV-tGOulwoDTE","clientExtensions":{},"hashAlgorithm":"SHA-256","origin":"http://localhost:9005","type":"webauthn.get"})
    ...
    static let accountId = "d3b68b92d654b38c026128af0a8dd8cd3a5c08a518b17762cf44eb410aa14e3e"
    ...
    static func createSeed() {
        ...
        do {            
            ...
            try Keychain.shared.save(id: KeyIdentifier.password.identifier(for: .passwordSeed), service: .seed, secretData: passwordSeed)
    ...
    static func createSharedKey() -> (Data, Data, Data) {
        ...
        do {
            ...
            try Keychain.shared.save(id: sharedKeyID, service: .browserSession(attribute: .shared), secretData: sharedKey)
            try Keychain.shared.save(id: signingPrivKeyID, service: .browserSession(attribute: .signing), secretData: privKey)
    ...
    static func createSessionInKeychain() -> Session {
        ...
        do {
            ...
            try Keychain.shared.save(id: sharedKeyID, service: .browserSession(attribute: .shared), secretData: sharedKey, objectData: encoder.encode(session))
            try Keychain.shared.save(id: signingPrivKeyID, service: .browserSession(attribute: .signing), secretData: privKey)
            return session
        } catch {
            fatalError(error.localizedDescription)
