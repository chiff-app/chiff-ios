...
class TestHelper {
    ...
    static let keynRequest = KeynRequest(accountID: TestHelper.userID, browserTab: 0, password: "[jh6eAX)og7A#nJ1:YDSrD6#61cf${\"A", passwordSuccessfullyChanged: false, siteID: TestHelper.sampleSite.id, siteName: TestHelper.sampleSite.name, siteURL: TestHelper.sampleSite.url, type: .add, username: TestHelper.username, sentTimestamp: TimeInterval(), count: 0, sessionID: TestHelper.sessionID, accounts: nil)
