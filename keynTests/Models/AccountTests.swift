...
class AccountTests: XCTestCase {
    ...
    func testSynced() {
        do {
            let account = try Account(username: TestHelper.username, sites: [TestHelper.sampleSite], passwordIndex: 0, password: nil)
    ...
    func testInitDoesntThrow() {
        XCTAssertNoThrow(try Account(username: TestHelper.username, sites: [TestHelper.sampleSite], passwordIndex: 0, password: nil))
        XCTAssertNoThrow(try Account(username: TestHelper.username + "2", sites: [TestHelper.sampleSite], passwordIndex: 0, password: "password"))
    }
    
    func testNextPassword() {
        let site = TestHelper.sampleSite
        do {
            var account = try Account(username: TestHelper.username, sites: [site], passwordIndex: 0, password: nil)
    ...
    func testOneTimePasswordToken() {
        let site = TestHelper.sampleSite
        do {
            let account = try Account(username: TestHelper.username, sites: [site], passwordIndex: 0, password: nil)
    ...
    func testSetOtpDoesntThrow() {
        ...
        do {
            var account = try Account(username: TestHelper.username, sites: [TestHelper.sampleSite], passwordIndex: 0, password: nil)
        ...
        do {
            ...
            account.delete { (result) in
                do {
                    ...
                    XCTAssertNil(try Account.get(accountID: accountId, context: nil))
                } catch {
                    XCTFail(error.localizedDescription)
                }
                expectation.fulfill()
    ...
    func testAll() {
        XCTAssertNoThrow(try Account.all(context: nil))
        XCTAssertTrue(try Account.all(context: nil).isEmpty)
        XCTAssertNoThrow(try Account(username: TestHelper.username, sites: [TestHelper.sampleSite], passwordIndex: 0, password: nil))
        XCTAssertNoThrow(try Account.all(context: nil))
        XCTAssertFalse(try Account.all(context: nil).isEmpty)
    }
    ...
    func testSave() {
        ...
        XCTAssertNoThrow(try Account.save(accountData: accountData, id: TestHelper.userID, context: nil))
        XCTAssertNotNil(try Account.get(accountID: TestHelper.userID, context: nil))
    }
    ...
    func testDeleteAll() {
        XCTAssertNoThrow(try Account(username: TestHelper.username, sites: [TestHelper.sampleSite], passwordIndex: 0, password: nil))
        XCTAssertNoThrow(try Account(username: TestHelper.username + "2", sites: [TestHelper.sampleSite], passwordIndex: 0, password: nil))
        Account.deleteAll()
        XCTAssertEqual(try Account.all(context: nil).count, 0)
    }
