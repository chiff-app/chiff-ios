...
class StoreObserver: NSObject {
    ...
    func updateSubscriptions(completionHandler: @escaping (Result<Void, Error>) -> Void) {
        do {
            API.shared.signedRequest(method: .get, message: nil, path: "subscriptions/\(try BackupManager.shared.publicKey())", privKey: try BackupManager.shared.privateKey(), body: nil) { result in
                switch result {
    ...
    private func validateReceipt(completionHandler: @escaping (Result<ValidationResult,Error>) -> Void) {
        if let appStoreReceiptURL = Bundle.main.appStoreReceiptURL, FileManager.default.fileExists(atPath: appStoreReceiptURL.path) {
            do {
                ...
                API.shared.signedRequest(method: .post, message: nil, path: "subscriptions/ios/\(try BackupManager.shared.publicKey())", privKey: try BackupManager.shared.privateKey(), body: jsonData) { result in
                    switch result {
                    case .success(let jsonObject):
